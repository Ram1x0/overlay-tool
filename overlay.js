...
const STALE_EVENT_MS = 15000; // ページ読み込み前からある古いイベントは再生しない
...
function enqueueGiftEvent(event) {
  if (!event) return;
  // ページを開く前から残っていた古いイベント(過去分の再生)は無視する
  if (typeof event.ts === 'number' && event.ts < pageLoadedAt - STALE_EVENT_MS) return;
