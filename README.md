...
- **キル数増減の演出**: チェックを入れると、増減時に画面中央にルーレット風の
  数字演出を表示する（数値そのものは常にキル増減の設定通りで、演出は見た目だけ）
...
- ギフト受信イベント演出の表示時間・ルーレットの速さを変えたい場合は、`overlay.js` の
  `EVENT_DISPLAY_MS` / `ROULETTE_SPIN_MS` / `ROULETTE_TICK_MS` を書き換えてください。
